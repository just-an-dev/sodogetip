...
def get_user_spendable_balance(address, rpc=None):
    ...
    for i in range(0, len(list_unspent), 1):
        ...
        # filter only vout with type "pubkeyhash"
        vout = filter(lambda x: x['scriptPubKey']['type'] == "pubkeyhash", vin['vout'])

        if vout[0]['scriptPubKey']['addresses'][0] in models.UserStorage.get_all_users_address().values():
